...
<div>
    <h2 class="text-xl font-bold text-gray-900 mb-1">{{ __('app.welcome_back') }}</h2>
    <p class="text-sm text-gray-500 mb-6">{{ __('app.login_subtitle') }}</p>
    ...
    <form wire:submit="login" class="space-y-5">
        ...
        <div class="flex items-center justify-between pt-2">
            @if (Route::has('password.request'))
                <a class="text-sm text-gray-600 hover:text-emerald-600 underline" href="{{ route('password.request') }}" wire:navigate>
        ...
        <p class="text-sm text-gray-500 text-center pt-2">
            {{ __('app.no_account') }}
            <a href="{{ route('register') }}" wire:navigate class="text-emerald-600 font-medium hover:underline">{{ __('app.register') }}</a>
        </p>
    </form>
</div>